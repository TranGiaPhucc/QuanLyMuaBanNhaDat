...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $conn->prepare("
            SELECT 
                nd.ten_nhadat, hd.ngaylap, cthd.soluong, cthd.giaca
            FROM 
                hoadon hd
            JOIN 
                chitiethoadon cthd ON hd.id_hoadon = cthd.id_hoadon
            JOIN 
                nhadat nd ON cthd.id_nhadat = nd.id_nhadat
            WHERE 
                MONTH(hd.ngaylap) = :month AND YEAR(hd.ngaylap) = :year
        ");
...
<!-----Main------>
	<div class="container-fluid">
        ...
        <a class="list-group-item active" href="TrangBieuDoThongKe.php">Thông Kê Doanh Số </a>
		...
		<d class="col-xl-9 col-lg-9 col-md-12 col-12">
			
	
    <h2 class="mt-4">Thống kê doanh thu</h2>
        ...
        <table class="table table-bordered mt-4">
            ...
            <tbody>
                ...
                foreach ($results as $row):
                    $total_amount += $row['soluong'] * $row['giaca'];
                ?>
                <tr>
                    <td><?php echo htmlspecialchars($row['ten_nhadat']); ?></td>
                    ...
                    <td><?php echo number_format($row['soluong'] * $row['giaca']); ?></td>
                </tr>
                <?php endforeach; ?>
            </tbody>
            <tfoot>
