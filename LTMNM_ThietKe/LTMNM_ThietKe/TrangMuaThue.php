...
// Lấy maloaihinh từ tham số URL
// Lấy tham số maloaihinh từ URL
$maloaihinh = isset($_GET['maloaihinh']) ? $_GET['maloaihinh'] : null;

// Tạo truy vấn SQL ban đầu
$sql = "SELECT nhadat.*, loai_hinh.tenloaihinh
        FROM nhadat
        INNER JOIN loai_hinh ON nhadat.maloaihinh = loai_hinh.maloaihinh";

// Nếu có maloaihinh, thêm điều kiện WHERE vào truy vấn
if ($maloaihinh !== null) {
    $sql .= " WHERE nhadat.maloaihinh = :maloaihinh";
}

try {
    $stmt = $conn->prepare($sql);

    // Nếu có maloaihinh, truyền giá trị vào truy vấn
    if ($maloaihinh !== null) {
  ...
  if ($stmt->rowCount() > 0) {
   ...
   //   $diachi = $row['id_diachi']; // Thay đổi cột tương ứng trong cơ sở dữ liệu
      ...
      $tenloaihinh = $row['tenloaihinh']; // Lấy tên loại hình
      $propertyCount++;
      ...
      echo '<span class="tag">' . $tenloaihinh . '</span>'; //
    ...
    if ($page > 1) {
      echo '<li class="page-item"><a class="page-link" href="?page='.($page - 1).'&maloaihinh='.$maloaihinh.'">Previous</a></li>';
    } else {
      echo '<li class="page-item disabled"><span class="page-link">Previous</span></li>';
    }
    for ($i = 1; $i <= $total_pages; $i++) {
      echo '<li class="page-item '.($page == $i ? 'active' : '').'"><a class="page-link" href="?page='.$i.'&maloaihinh='.$maloaihinh.'">'.$i.'</a></li>';
    }
    if ($page < $total_pages) {
      echo '<li class="page-item"><a class="page-link" href="?page='.($page + 1).'&maloaihinh='.$maloaihinh.'">Next</a></li>';
    } else {
