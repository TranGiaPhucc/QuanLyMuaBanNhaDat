...
// Lấy thông tin cá nhân từ bảng khachhang
$sql_customer = "SELECT * FROM khachhang WHERE username = :username";
...
// Lấy thông tin về địa chỉ từ bảng province, wards, district
$sql_address = "SELECT province.name AS province_name, district.name AS district_name, wards.name AS ward_name 
               FROM province 
               INNER JOIN district ON province.province_id = district.province_id 
               INNER JOIN wards ON district.district_id = wards.district_id 
               WHERE province.province_id = :province_id AND district.district_id = :district_id AND wards.wards_id = :wards_id";
$stmt_address = $conn->prepare($sql_address);
$stmt_address->bindParam(':province_id', $customer['province_id']);
$stmt_address->bindParam(':district_id', $customer['district_id']);
$stmt_address->bindParam(':wards_id', $customer['wards_id']);
$stmt_address->execute();
$address_info = $stmt_address->fetch(PDO::FETCH_ASSOC);

// Gán giá trị cho các biến tương ứng
$id = $customer['id_khachhang'];
$name = $customer['hoten_kh'];
$gender = ($customer['gioitinh'] == 0) ? "Nữ" : "Nam";
$email = $user['email'];
$phone = $customer['sdt_kh'];
$province_name = $address_info['province_name'];
$district_name = $address_info['district_name'];
$ward_name = $address_info['ward_name'];
}
?>
...
<!-----Main------>
    ...
    <div class="col-xl-3 col-lg-3 col-md-4 col-6">
		...
		<nav class="list-group">
                <a class="list-group-item active" href="TrangTTCN_KH.php"> Tài Khoản</a>
    ...
    <article class="card mb-3">
        <div class="card-body">
            <figure class="icontext">
                <div class="icon">
                    <img class="rounded-circle img-sm border" src="images/avatars/sancho.jpg">
                </div>
                <div class="text">
                    <!-- Thay thế các biến bằng dữ liệu thực tế từ cơ sở dữ liệu -->
                    <strong> ID: <?php echo $id; ?></strong> <br>
                    <strong> Họ Và Tên: <?php echo $name; ?></strong> <br>
                    <strong> Giới Tính: <?php echo $gender; ?></strong> <br>
                    <strong> Email: <?php echo $email; ?></strong> <br>
                    <strong> Điện Thoại: <?php echo $phone; ?></strong> <br>
                    <strong> Tài Khoản </strong> <br>
                    <p class="mb-2"> UserName: <?php echo $username; ?></p>
					<p class="mb-2"> Password: <?php echo $username; ?></p>
                    <a href="#" class="btn btn-light btn-sm">Cập Nhập Thông Tin</a>
                </div>
            </figure>
            <hr>
            <p>
                ...
                <br>
                <p class="mb-2"> Địa Chỉ: <?php echo $address_info['province_name'] . ', ' . $address_info['district_name'] . ', ' . $address_info['ward_name']; ?></p> &nbsp
