...
// Kiểm tra xem có ID khách hàng được truyền qua URL không
if (isset($_GET['sid'])) {
    ...
    try {
        ...
        // Truy vấn để lấy chi tiết khách hàng và tên địa chỉ từ các bảng liên quan
        $stmt = $conn->prepare("
            SELECT 
                kh.id_khachhang, kh.username, kh.hoten_kh, kh.ngaysinh, kh.gioitinh, kh.sdt_kh, 
                d.name AS district_name, p.name AS province_name, w.name AS wards_name, kh.Thon_SoNha, kh.TinhTrang
            FROM 
                khachhang kh
            JOIN 
                district d ON kh.district_id = d.district_id
            JOIN 
                province p ON kh.province_id = p.province_id
            JOIN 
                wards w ON kh.wards_id = w.wards_id
            WHERE 
...
<div class="container">
<h2 class="mt-4">Chi tiết khách hàng</h2>
    <?php if (isset($error)): ?>
    ...
    <?php else: ?>
        <table class="table table-bordered mt-4">
            <tr>
                <th>ID Khách Hàng</th>
                <td><?php echo htmlspecialchars($row['id_khachhang']); ?></td>
            </tr>
            <tr>
                <th>Username</th>
            ...
            <tr>
                <th>Họ Tên</th>
                <td><?php echo htmlspecialchars($row['hoten_kh']); ?></td>
            </tr>
            <tr>
                <th>Ngày Sinh</th>
            ...
            <tr>
                <th>SĐT</th>
                <td><?php echo htmlspecialchars($row['sdt_kh']); ?></td>
            </tr>
            <tr>
                <th>Thôn/Số Nhà</th>
