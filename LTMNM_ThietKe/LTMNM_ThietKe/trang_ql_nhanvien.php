...
function paginate($page, $total_pages) {
    ...
    if ($total_pages > 1) {
        ...
        if ($page > 1) {
            $pagination .= '<li class="page-item"><a class="page-link" href="?page='.($page-1).'">&laquo;</a></li>';
        }
        for ($i = 1; $i <= $total_pages; $i++) {
            $pagination .= '<li class="page-item '.($page == $i ? 'active' : '').'"><a class="page-link" href="?page='.$i.'">'.$i.'</a></li>';
        }
        if ($page < $total_pages) {
            $pagination .= '<li class="page-item"><a class="page-link" href="?page='.($page+1).'">&raquo;</a></li>';
        }
        $pagination .= '</ul>';
    }
    return $pagination;
...
try {
    ...
    $total_records = $conn->query("SELECT COUNT(*) AS total FROM nhanvien WHERE TinhTrang != 'Khong Ton Tai'")->fetchColumn();
    $total_pages = ceil($total_records / $limit); // Tính tổng số trang

    // Truy vấn để lấy dữ liệu cho trang hiện tại
    $stmt = $conn->prepare("SELECT * FROM nhanvien WHERE TinhTrang != 'Khong Ton Tai' LIMIT :start, :limit");
		...
		<div class="row">
			<div class="col-xl-3 col-lg-3 col-md-4 col-6">
                ...
                <nav class="list-group">
                    ...
                    <a class="list-group-item active" href="trang_ql_nhanvien.php">Quản Lý Nhân Viên</a>
...
<br>
    <table class="table">
        ...
        <tbody>
			...
			<div class="form-group col-md">
			  <a href="editNV.php?sid=<?php echo $row['Id_NhanVien']; ?>" class="btn btn-danger">
        <i class=""></i> <span class="text">Edit</span> </a>


				<a href="detailNV.php?sid=<?php echo $row['Id_NhanVien']; ?>"  class="btn  btn-danger"> 
				<i class=""></i> <span class="text">Details</span> 
			</a>

      <a href="xoaNV.php?sid=<?php echo $row['Id_NhanVien']; ?>" class="btn btn-danger" onclick="return confirm('Bạn có muốn Xoá Không?');">Delete</a>
    ...
    <?php echo paginate($page, $total_pages); // Hiển thị phân trang ?>
    </div> <!-- card-body .// -->
</article> 
			</div>
...
<div class="modal" id="myModal">
    ...
    <div class="form-group">
        <label for="hoten_nv">Họ tên</label>
