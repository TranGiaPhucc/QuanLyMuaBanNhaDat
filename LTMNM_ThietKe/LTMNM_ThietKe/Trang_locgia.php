...
// Lấy tham số giá từ URL
$priceRange = isset($_GET['price_range']) ? $_GET['price_range'] : '100000000-500000000';

// Split giá range thành giá trị min và max
$priceFilters = explode('-', $priceRange);
$minPrice = $priceFilters[0];
$maxPrice = $priceFilters[1];

// Tạo truy vấn SQL để lọc nhà đất theo giá
$sql = "SELECT * FROM nhadat WHERE gia BETWEEN :minPrice AND :maxPrice";
...
if ($stmt->rowCount() > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
      ...
      echo '<span class="h5 price"> Diện Tích : '. $dientich . '</span>';
      echo '<small class="text-muted">m²</small>';
    ...
    if ($page > 1) {
      echo '<li class="page-item"><a class="page-link" href="?page='.($page - 1).'&price_range='.$priceRange.'">Previous</a></li>';
    } else {
      echo '<li class="page-item disabled"><span class="page-link">Previous</span></li>';
    }
    for ($i = 1; $i <= $total_pages; $i++) {
      echo '<li class="page-item '.($page == $i ? 'active' : '').'"><a class="page-link" href="?page='.$i.'&price_range='.$priceRange.'">'.$i.'</a></li>';
    }
    if ($page < $total_pages) {
      echo '<li class="page-item"><a class="page-link" href="?page='.($page + 1).'&price_range='.$priceRange.'">Next</a></li>';
    } else {
