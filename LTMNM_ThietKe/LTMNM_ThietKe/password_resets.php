...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Kiểm tra xem 'token' đã được truyền qua POST hay không
    if (isset($_POST['token'])) {
        ...
        try {
            ...
            if ($reset && strtotime($reset['expiry']) > time()) {
                // Người dùng đã nhập đúng mã xác thực, chuyển hướng đến trang đổi mật khẩu
        ...
        // Trường 'token' không tồn tại trong POST
...
<div class="card-body">
  <h4 class="card-title mb-4 text-center">Enter Reset Code</h4>
  <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
    <div class="form-group">
      <input name="token" class="form-control" id="token" placeholder="Reset Code" type="text" required>
    </div>
    <div class="form-group">
